...
const FilterModel = ({ selectedFilters, onFilterChange, onClose }) =>{
    const [priceRange, setPriceRange] = useState({
        min: selectedFilters.setPriceRange?.min || 600,
        max: selectedFilters.setPriceRange?.max || 30000,
    });
    ...
    const [amenities, setAmenities] = useState(selectedFilters.amenities || []); // amenities are empty arrays
                ...
                <div className="icon-box">

                    {
                        roomTypeOptions.map((option)=>(
                            ...
                            >
                                <span className="materials-icon">{option.icon}</span>
